...
public class CompositeCollector {
    ...
    public static CrateCollector newInstance(Collection<? extends CrateCollector.Builder> builders, RowReceiver rowReceiver) {
        List<BatchIterator> batchIterators = new ArrayList<>();
        for (CrateCollector.Builder builder : builders) {
            batchIterators.add(Objects.requireNonNull(
                builder.createBatchIterator(),
                "CrateCollector.Builder must be able to create a BatchIterator in order to support composite collection"));
        }
