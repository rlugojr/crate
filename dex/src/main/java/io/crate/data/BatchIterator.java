...
public interface BatchIterator {
    ...
    /**
     * This method returns an input list object which can be used to access the underlying data of the current
     * iterator position.
     *
     * Note that it is only valid to call {@link Input#value()} on any of the returned inputs if the iterator is
     ...
     * iterator or its position. However it is good practice for consumers of this iterator to gather the inputs
     * before iterating. This method is required to always return the same object on every call.
     *
     * @return an input list object
     */
    InputList rowData();
